...
void UART_Init(uint24_t baudRate) {
    ...
    {
        SPBRG = (uint8_t)((16000000UL) / (baudRate * 64)) - 1;
    }
    else if(!(TXSTAbits.SYNC) && !(BAUDCONbits.BRG16) && (TXSTAbits.BRGH))
    {
        temp = (uint8_t)(((16000000UL) / (baudRate * 16)) - 1);
        SPBRGH = (uint8_t)((temp & 0xFF00) >> 8);
        SPBRG = (uint8_t) (temp & 0x00FF);
    }else if(!(TXSTAbits.SYNC) && (BAUDCONbits.BRG16) && (TXSTAbits.BRGH))
    {
        temp = (uint8_t)(((16000000UL) / (baudRate * 4)) - 1);
        SPBRGH = (uint8_t)((temp & 0xFF00) >> 8);
        SPBRG = (uint8_t) (temp & 0x00FF);
    }
    RCSTA = 0b10010000;
...
uint16_t UART_available(void)
{
    return ((uint16_t) (512 + _rx_buffer_head - _rx_buffer_tail)) % 512;
}
int8_t UART_peek(void)
{
...
int8_t UART_read(void) {
    ...
    if (_rx_buffer_head == _rx_buffer_tail) {
        ...
        _rx_buffer_tail = (rx_buffer_index_t) (_rx_buffer_tail + 1) % 512;
        return (int8_t)c;
    }
}
void UART_end(void)
...
void UART_rx_complete_irq(void) {
    ...
    c = RCREG;
    rx_buffer_index_t i = (unsigned int) (_rx_buffer_head + 1) % 512;
